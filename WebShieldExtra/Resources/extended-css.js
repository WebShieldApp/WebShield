...
var ExtendedCss = (() => {
    ...
    var REGEXP_VALID_OLD_SYNTAX = /\[-(?:ext)-([a-z-_]+)=(["'])((?:(?=(\\?))\4.)*?)\2\]/g;
    var INVALID_OLD_SYNTAX_MARKER = "[-ext-";
    var evaluateMatch = (match, name, quoteChar, rawValue) => {
    ...
    var tokenize = (input, supportedMarks) => {
        let wordBuffer = "";
        const tokens = [];
    ...
    var tokenizeSelector = (rawSelector) => {
        const selector = convert(rawSelector);
        return tokenize(selector, SUPPORTED_SELECTOR_MARKS);
    };
    var tokenizeAttribute = (attribute) => { return tokenize(attribute, [...SUPPORTED_SELECTOR_MARKS, EQUAL_SIGN ]); };
    ...
    var getFirst = (array) => { return array[0]; };
    var getLast = (array) => { return array[array.length - 1]; };
    var getPrevToLast = (array) => { return array[array.length - 2]; };
    var getItemByIndex = (array, index, errorMessage) => {
    ...
    var NO_REGULAR_SELECTOR_ERROR = "At least one of Selector node children should be RegularSelector";
    var isSelectorListNode = (astNode) => { return astNode?.type === NODE.SELECTOR_LIST; };
    var isSelectorNode = (astNode) => { return astNode?.type === NODE.SELECTOR; };
    var isRegularSelectorNode = (astNode) => { return astNode?.type === NODE.REGULAR_SELECTOR; };
    var isExtendedSelectorNode = (astNode) => { return astNode.type === NODE.EXTENDED_SELECTOR; };
    var isAbsolutePseudoClassNode = (astNode) => { return astNode?.type === NODE.ABSOLUTE_PSEUDO_CLASS; };
    var isRelativePseudoClassNode = (astNode) => { return astNode?.type === NODE.RELATIVE_PSEUDO_CLASS; };
    var getNodeName = (astNode) => {
        if (astNode === null) {
    ...
    var getNodeValue = (astNode, errorMessage) => {
        if (astNode === null) {
            throw new Error("Ast node should be defined");
        }
        ...
        if (!astNode.value) {
            throw new Error(
                errorMessage || "Ast RegularSelector ot AbsolutePseudoClass node should have a value",
            );
        }
        return astNode.value;
    };
    var getRegularSelectorNodes = (children) => { return children.filter(isRegularSelectorNode); };
    var getFirstRegularChild = (children, errorMessage) => {
    ...
    var getLastRegularChild = (children) => {
        const regularSelectorNodes = getRegularSelectorNodes(children);
    ...
    var getNodeOnlyChild = (node, errorMessage) => {
    ...
    var getPseudoClassNode = (extendedSelectorNode) => {
        return getNodeOnlyChild(
            extendedSelectorNode,
            "Extended selector should be specified",
        );
    };
    var getRelativeSelectorListNode = (pseudoClassNode) => {
    ...
    var isSupportedPseudoClass = (tokenValue) => { return SUPPORTED_PSEUDO_CLASSES.includes(tokenValue); };
    var isOptimizationPseudoClass = (name) => { return OPTIMIZATION_PSEUDO_CLASSES.includes(name); };
    var doesRegularContinueAfterSpace = (nextTokenType, nextTokenValue) => {
        ...
        return (COMBINATORS.includes(nextTokenValue) || nextTokenType === TOKEN_TYPE.WORD ||
                ...
                nextTokenValue === COLON || nextTokenValue === SINGLE_QUOTE || nextTokenValue === DOUBLE_QUOTE ||
                nextTokenValue === BRACKET.SQUARE.LEFT);
    };
    var isRegexpOpening = (context, prevTokenValue, bufferNodeValue) => {
    ...
    var isAttributeOpening =
        (tokenValue, prevTokenValue) => { return tokenValue === BRACKET.SQUARE.LEFT && prevTokenValue !== BACKSLASH; };
    var isAttributeClosing = (context) => {
        ...
        if (firstAttrTokenType === TOKEN_TYPE.MARK && firstAttrTokenValue !== BACKSLASH) {
            throw new Error(
        ...
        const equalSignIndex =
            attrTokens.findIndex((token) => { return token.type === TOKEN_TYPE.MARK && token.value === EQUAL_SIGN; });
        ...
        if (equalSignIndex === -1) {
            ...
            return (prevToLastAttrTokenValue === BACKSLASH &&
                    (lastAttrTokenValue === DOUBLE_QUOTE || lastAttrTokenValue === SINGLE_QUOTE));
        }
        ...
        const isAttrValueQuote =
            nextToEqualSignTokenValue === SINGLE_QUOTE || nextToEqualSignTokenValue === DOUBLE_QUOTE;
        if (!isAttrValueQuote) {
            if (lastAttrTokenType === TOKEN_TYPE.WORD) {
                return true;
            }
    ...
    var isWhiteSpaceChar = (tokenValue) => {
        if (!tokenValue) {
            return false;
        }
    ...
    var isAbsolutePseudoClass = (str) => { return ABSOLUTE_PSEUDO_CLASSES.includes(str); };
    var isRelativePseudoClass = (str) => { return RELATIVE_PSEUDO_CLASSES.includes(str); };

    // src/selector/utils/parser-ast-node-helpers.ts
    var getBufferNode = (context) => {
        if (context.pathToBufferNode.length === 0) {
            return null;
        }
        return getLast(context.pathToBufferNode) || null;
    };
    var getBufferNodeParent = (context) => {
        if (context.pathToBufferNode.length < 2) {
            return null;
        }
        return getPrevToLast(context.pathToBufferNode) || null;
    };
    var getContextLastRegularSelectorNode = (context) => {
    ...
    var updateBufferNode = (context, tokenValue) => {
        const bufferNode = getBufferNode(context);
        if (bufferNode === null) {
    ...
    var addSelectorListNode = (context) => {
    ...
    var addAstNodeByType = (context, type, tokenValue = "") => {
        const bufferNode = getBufferNode(context);
        if (bufferNode === null) {
    ...
    var initAst = (context, tokenValue) => {
    ...
    var initRelativeSubtree = (context, tokenValue = "") => {
        addAstNodeByType(context, NODE.SELECTOR_LIST);
        addAstNodeByType(context, NODE.SELECTOR);
        addAstNodeByType(context, NODE.REGULAR_SELECTOR, tokenValue);
    };
    var upToClosest = (context, parentType) => {
    ...
    var getUpdatedBufferNode = (context) => {
        const bufferNode = getBufferNode(context);
        ...
        if (supposedPseudoClassNode) {
            ...
            const hasRelativeExtended =
                isLastExtendedNameRelative && context.extendedPseudoBracketsStack.length > 0 &&
    ...
    var handleNextTokenOnColon = (
        ...
        if (!isSupportedPseudoClass(nextTokenValue.toLowerCase())) {
            ...
            if (HAS_PSEUDO_CLASS_MARKERS.includes(nextTokenValue) && context.standardPseudoNamesStack.length > 0) {
                throw new Error(
                    `Usage of :${nextTokenValue}() pseudo-class is not allowed inside regular pseudo: '${
    ...
    var hasExtendedSelector = (selectorList) => {
        return selectorList.children.some((selectorNode) => {
            return selectorNode.children.some(
                (selectorNodeChild) => { return isExtendedSelectorNode(selectorNodeChild); });
        });
    };
    var selectorListOfRegularsToString = (selectorList) => {
    ...
    var updateNodeChildren = (node, newChildren) => {
        node.children = newChildren;
        return node;
    };
    var shouldOptimizeExtendedSelector = (currExtendedSelectorNode) => {
    ...
    var getOptimizedExtendedSelector = (
        currExtendedSelectorNode,
        ...
        if (!hasInnerExtendedSelector) {
            ...
            const pseudoName = getNodeName(extendedPseudoClassNode);
            const optimizedExtendedStr = `${COLON}${pseudoName}${BRACKET.PARENTHESES.LEFT}${relativeSelectorListStr}${
                BRACKET.PARENTHESES.RIGHT}`;
    ...
    var optimizeCurrentRegularSelector =
        (current, previous) => { previous.value = `${getNodeValue(previous)}${SPACE}${getNodeValue(current)}`; };
    var optimizeSelectorNode = (selectorNode) => {
    ...
    var optimizeSelectorListNode = (selectorListNode) => {
        return updateNodeChildren(
            selectorListNode,
            selectorListNode.children.map((s) => optimizeSelectorNode(s)),
        );
    };
    var optimizeAst = (ast) => { return optimizeSelectorListNode(ast); };

    // src/selector/parser.ts
    var XPATH_PSEUDO_SELECTING_ROOT = "body";
    var NO_WHITESPACE_ERROR_PREFIX = "No white space is allowed before or after extended pseudo-class name in selector";
    var parse = (selector) => {
                ...
                case BRACKET.SQUARE.RIGHT:
                    if (isRegularSelectorNode(bufferNode)) {
                        if (!context.isAttributeBracketsOpen && prevTokenValue !== BACKSLASH) {
                            throw new Error(
                                `'${selector}' is not a valid selector due to '${tokenValue}' after '${
                                    getNodeValue(bufferNode)}'`,
                            );
                        }
                    ...
                    break;
                case COLON:
                    if (isWhiteSpaceChar(nextTokenValue) && nextToNextTokenValue &&
                    ...
                    if (isAbsolutePseudoClassNode(bufferNode)) {
                        if (getNodeName(bufferNode) === XPATH_PSEUDO_CLASS_MARKER && nextTokenValue &&
                    ...
                    if (isRegularSelectorNode(bufferNode)) {
                        if (context.isAttributeBracketsOpen) {
                            ...
                            if (Object.values(REGULAR_PSEUDO_ELEMENTS)
                                ...
                                nextTokenValue === COLON && nextToNextTokenValue &&
    ...
    var isHtmlElement = (element) => { return element instanceof HTMLElement; };
    var getParent = (element, errorMessage) => {
    ...
    var getErrorMessage = (error) => { return toErrorWithMessage(error).message; };

    // src/common/utils/logger.ts
    var logger = {
        error : typeof console !== "undefined" && console.error && console.error.bind
                    ? console.error.bind(window.console)
                    : console.error,
        info : typeof console !== "undefined" && console.info && console.info.bind ? console.info.bind(window.console)
                                                                                   : console.info,
    };

    // src/common/utils/strings.ts
    var removeSuffix = (str, suffix) => {
    ...
    var replaceAll = (input, pattern, replacement) => {
        if (!input) {
            return input;
        }
    ...
    var convertTypeIntoString = (value) => {
        let output;
        switch (value) {
        case void 0:
            output = "undefined";
    ...
    var convertTypeFromString = (value) => {
    ...
    var isUserAgentSupported = (userAgent) => {
        if (userAgent.includes("MSIE") || userAgent.includes("Trident/")) {
            return false;
        }
        return true;
    };
    var isBrowserSupported = () => { return isUserAgentSupported(navigator.userAgent); };
    ...
    var removeContentQuotes = (str) => { return str.replace(/^(["'])([\s\S]*)\1$/, "$2"); };
    var addUrlPropertyQuotes = (str) => {
        if (!str.includes('url("')) {
            const re = /url\((.*?)\)/g;
    ...
    var convertStyleMatchValueToRegexp = (rawValue) => {
        let value;
    ...
    var normalizePropertyValue = (propertyName, propertyValue) => {
        let normalized = "";
        switch (propertyName) {
        ...
        case CSS_PROPERTY.OPACITY:
            normalized =
                isSafariBrowser ? (Math.round(parseFloat(propertyValue) * 100) / 100).toString() : propertyValue;
            break;
        default:
    ...
    var getPseudoArgData = (pseudoArg, separator) => {
    ...
    var parseStyleMatchArg = (pseudoName, rawArg) => {
    ...
    var getValidMatcherArg = (rawArg, isWildcardAllowed = false) => {
        let arg;
    ...
    var getRawMatchingData = (pseudoName, pseudoArg) => {
    ...
    var isAttributeMatched = (argsData) => {
        const {pseudoName, pseudoArg, domElement} = argsData;
        ...
        while (i < elementAttributes.length && !isMatched) {
            ...
            const isNameMatched =
                attrNameMatch instanceof RegExp ? attrNameMatch.test(attr.name) : attrNameMatch === attr.name;
            if (!rawAttrValue) {
                ...
                const isValueMatched =
                    attrValueMatch instanceof RegExp ? attrValueMatch.test(attr.value) : attrValueMatch === attr.value;
    ...
    var filterRootsByRegexpChain = (base, chain, output = []) => {
    ...
    var isPropertyMatched = (argsData) => {
        const {pseudoName, pseudoArg, domElement} = argsData;
    ...
    var getValidNumberAncestorArg = (rawArg, pseudoName) => {
    ...
    var getNthAncestor = (domElement, nth, pseudoName) => {
        let ancestor = null;
        let i = 0;
        while (i < nth) {
    ...
    var validateStandardSelector = (selector) => {
        let isValid;
        try {
    ...
    var matcherWrapper = (callback, argsData, errorMessage) => {
        let isMatched;
        try {
    ...
    var getAbsolutePseudoError = (propDesc, pseudoName, pseudoArg) => {
        return `${MATCHING_ELEMENT_ERROR_PREFIX} ${propDesc}, may be invalid :${pseudoName}() pseudo-class arg: '${
            pseudoArg}'`;
    };
    var isMatchedByAbsolutePseudo = (domElement, pseudoName, pseudoArg) => {
        let argsData;
        let errorMessage;
        let callback;
        switch (pseudoName) {
    ...
    var isAnyElementBySelectorList = (argsData) => {
        const {element, relativeSelectorList, pseudoName} = argsData;
    ...
    var notElementBySelectorList = (argsData) => {
        const {element, relativeSelectorList, pseudoName} = argsData;
        return relativeSelectorList.children.every((selectorNode) => {
            const relativeRegularSelector = getFirstInnerRegularChild(
                selectorNode,
    ...
    var getByRegularSelector = (regularSelectorNode, root, specifiedSelector) => {
    ...
    var getByExtendedSelector = (domElements, extendedSelectorNode) => {
        let foundElements = [];
    ...
    var getByFollowingRegularSelector = (domElements, regularSelectorNode) => {
        let foundElements = [];
    ...
    var getElementsForSelectorNode = (selectorNode, root, specifiedSelector) => {
        let selectedElements = [];
        let i = 0;
    ...
    var selectElementsByAst = (ast, doc = document) => {
        const selectedElements = [];
        ast.children.forEach(
            (selectorNode) => { selectedElements.push(...getElementsForSelectorNode(selectorNode, doc)); });
        const uniqueElements = [...new Set(flatten(selectedElements)) ];
        return uniqueElements;
    };
    var ExtCssDocument = class {
        constructor() { this.astCache = /* @__PURE__ */ new Map(); }
    ...
    var getObjectFromEntries = (entries) => {
        const object = {};
        entries.forEach((el) => {
            const [key, value] = el;
            object[key] = value;
    ...
    var parseRemoveSelector = (rawSelector) => {
        const VALID_REMOVE_MARKER =
            `${COLON}${REMOVE_PSEUDO_MARKER}${BRACKET.PARENTHESES.LEFT}${BRACKET.PARENTHESES.RIGHT}`;
    ...
    var parseSelectorRulePart = (selectorBuffer, extCssDoc) => {
    ...
    var createRawResultsMap = () => { return /* @__PURE__ */ new Map(); };
    var saveToRawResults = (rawResults, rawRuleData) => {
    ...
    var isRemoveSetInStyles = (styles) => {
        return styles.some(
            (s) => { return (s.property === REMOVE_PSEUDO_MARKER && s.value === PSEUDO_PROPERTY_POSITIVE_VALUE); });
    };
    var getDebugStyleValue = (styles) => {
        const debugStyle = styles.find((s) => { return s.property === DEBUG_PSEUDO_PROPERTY_KEY; });
        return debugStyle?.value;
    };
    var prepareRuleData = (rawRuleData) => {
        const {selector, ast, rawStyles} = rawRuleData;
        if (!ast) {
    ...
    var combineRulesData = (rawResults) => {
        const results = [];
    ...
    var tokenizeStyleBlock = (rawStyle) => {
    ...
    var isValueQuotesOpen = (context) => { return context.bufferValue !== "" && context.valueQuoteMark !== null; };
    var collectStyle = (context) => {
        context.styles.push({
    ...
    var processPropertyToken = (context, styleBlock, token) => {
    ...
    var processValueToken = (context, styleBlock, token) => {
        const {value : tokenValue} = token;
        if (token.type === TOKEN_TYPE.WORD) {
            ...
            case DOUBLE_QUOTE:
                ...
                } else if (!context.bufferValue.endsWith(BACKSLASH) && context.valueQuoteMark === tokenValue) {
    ...
    var parseStyleBlock = (rawStyleBlock) => {
    ...
    var getLeftCurlyBracketIndexes = (cssRule) => {
        const indexes = [];
    ...
    var parseRule = (rawCssRule, extCssDoc) => {
    ...
    var parseRules = (rawCssRules, extCssDoc) => {
  ...
  ${warnings.join("\n  ")}`);
    ...
    var restoreRuleAcc = (context) => {
        context.rawRuleData = {
            selector : "",
        };
    };
    var parseSelectorPart = (context, extCssDoc) => {
    ...
    var parseUntilClosingBracket = (context, styles) => {
    ...
    var parseStylesheet = (rawStylesheet, extCssDoc) => {
    ...
    var isNumber = (arg) => { return typeof arg === "number" && !Number.isNaN(arg); };
    ...
    var EventTracker = class {
        constructor() {
            this.getLastEventType = () => this.lastEventType;
            this.getTimeSinceLastEvent = () => {
                if (!this.lastEventTime) {
                    return null;
                }
                return Date.now() - this.lastEventTime;
            };
            this.trackedEvents = isSafariBrowser ? SUPPORTED_EVENTS.filter(
        ...
        isIgnoredEventType() {
            ...
            return (!!lastEventType && IGNORED_EVENTS.includes(lastEventType) && !!sinceLastEventTime &&
    ...
    function shouldIgnoreMutations(mutations) { return !mutations.some((m) => m.type !== "attributes"); }
    ...
    var removeElement = (context, affectedElement) => {
    ...
    var setStyleToElement = (node, style) => {
    ...
    var isIAffectedElement = (affectedElement) => {
        return ("node" in affectedElement && "rules" in affectedElement && affectedElement.rules instanceof Array);
    };
    var isAffectedElement = (affectedElement) => {
        return ("node" in affectedElement && "originalStyle" in affectedElement && "rules" in affectedElement &&
                affectedElement.rules instanceof Array);
    };
    var applyStyle = (context, rawAffectedElement) => {
    ...
    var revertStyle = (affectedElement) => {
    ...
    var createProtectionCallback = (styles) => {
    ...
    var protectStyleAttribute = (node, rules) => {
    ...
    var beautifyTimingNumber = (timestamp) => { return Number(timestamp.toFixed(STATS_DECIMAL_DIGITS_COUNT)); };
    var beautifyTimings = (rawTimings) => {
        return {
            appliesTimings : rawTimings.appliesTimings.map(
                (t) => beautifyTimingNumber(t),
                ),
    ...
    var findAffectedElement = (affElements, domNode) => { return affElements.find((affEl) => affEl.node === domNode); };
    var applyRule = (context, ruleData) => {
    ...
    var ExtendedCss = class {
        constructor(configuration) {
            ...
            this.context.debug = configuration.debug ||
                                 this.context.parsedRules.some(
                                     (ruleData) => { return ruleData.debug === DEBUG_PSEUDO_PROPERTY_GLOBAL_VALUE; });
        ...
        init() { nativeTextContent.setGetter(); }
        apply() {
