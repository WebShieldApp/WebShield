(() => {
    ...
    const handleBlockElement = async () => {
        ...
        runnerScript.addEventListener("zapper.runner-response",
                                      (event) => { safari.extension.dispatchMessage("ruleResponse", event.detail); });
    ...
    if (isHTMLDocument) {
        ...
        const onLinkClicked = (event) => {
            ...
            const urlParams = search ? search.slice(1) : href.slice(href.indexOf("?") + 1);
            const {title, url} = getSubscriptionParams(
                urlParams.replace(/&amp;/g, "&"),
            );
            ...
            safari.extension.dispatchMessage("addFilterSubscription", {
                url : url.trim(),
